...
def main():
    ...
    parser.add_argument(
        "--model", default="medium", choices=["small", "medium", "large-v3"]
    )
    ...
    for seg in segments:
        ...
        print(
            json.dumps(
                {
                    "progress": min(99, 100 * seg.end / max(info.duration, 1)),
                    "message": f"내용 분석용 음성 인식 {seg.end/60:.1f}/{info.duration/60:.1f}분",
                },
                ensure_ascii=False,
            ),
            flush=True,
