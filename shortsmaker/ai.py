...
목표: 문구만 읽어도 상황은 이해되고, 그 이유·과정·차이가 궁금해 영상을 보고 싶어야 합니다. 궁금증을 만드는 것과 의문문을 쓰는 것은 다릅니다.
...
문장 형식 다양성은 필수입니다. 10개 중 질문형은 0~2개까지만, 나머지 8개 이상은 비의문형으로 만드세요.
문장 중간의 반문이나 물음표 없는 의문형 종결도 질문형으로 셉니다. 물음표만 지워 제한을 피하지 마세요.
비의문형은 내용에 맞는 사실·발견, 공감·상황 묘사, 대비·반전, 행동 제안 등 최소 3가지 접근으로 나누세요.
어미만 바꾸거나 같은 질문을 평서문으로 치환하지 말고, 시청자가 주목할 장면·관점도 달리하세요.
단정형도 영상에서 확인되는 범위와 조건을 지키세요. 추천은 질문형을 우대하지 말고 내용 적합성과 볼 이유로 고르세요.
반환 전 10개의 문장 형식을 다시 세고 질문형이 2개를 넘으면 초과 후보를 다른 관점의 비의문형으로 다시 쓰세요.
...
- 사실·발견: '같은 무게가 가벼워진 이유 / 근육이 커져서만은 아닙니다'
- 대비·반전: '10개 들 수 있어도 / 5개에서 멈추는 선택'
- 공감·상황: '첫 세트에 쏟아부은 힘 / 다음 세트엔 반 토막'
- 행동 제안: '한 번 운동하면 며칠 쉬는 당신 / 한 번의 운동량부터 돌아보세요'
- 질문: '운동 두 가지, 5회씩 2세트 / 이걸로 충분할까요?'
