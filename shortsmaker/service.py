...
def new_clip(start,end,title='',reason=''):
    return dict(id=uid(),start=float(start),end=float(end),title=title,reason=reason,
                included=True,hooks=[],recommended_index=None,recommendation_reason='',
                hook='',yellow='',confirmed=False,font_size=80,framing=[],manual_frame=None,render=None)


def validate_clip(clip,duration):
    a,b=clip.get('start'),clip.get('end')
    if not all(isinstance(x,(int,float)) and not isinstance(x,bool) and math.isfinite(x) for x in (a,b)) or not 0<=a<b<=duration+.01:
    ...
    if frame and (not all(isinstance(frame.get(k),(float,int)) and math.isfinite(frame[k]) for k in ('zoom','center'))
                  or not 1<=frame['zoom']<=2 or not 0<=frame['center']<=1):
...
class Service:
    ...
    def source(self,p):
        ...
        s=path.stat()
        if s.st_size!=p['metadata']['size'] or s.st_mtime_ns!=p['metadata']['mtime_ns']:
            raise ValueError('불러온 뒤 원본 파일이 변경되었습니다. 새 프로젝트로 다시 불러와 주세요.')
        return path

    def selected(self,p,args):
        ids=args.get('clip_ids')
        result=[c for c in p['clips'] if c['included'] and (ids is None or c['id'] in ids)]
        if not result:
    ...
    def analyze(self,ctx,pid,args):
        ...
        if not transcript_file.exists():
            ctx.progress('내용 분석용 음성 인식 준비 중 · 첫 실행은 모델을 다운로드합니다.',0)
            def report(line):
                try:
                    v=json.loads(line)
                    if 'progress' in v: ctx.progress(v['message'],v['progress'])
                except (ValueError,TypeError):
                    pass
            ctx.run([sys.executable,'-m','shortsmaker.transcribe',p['source'],str(transcript_file),'--model',p['stt_model']],
                    cwd=Path(__file__).resolve().parent.parent,timeout=14400,on_line=report)
        ...
        clips=[new_clip(c['start'],c['end'],c['title'],c['reason']) for c in result['clips']]
        self.store.change(pid,lambda p:p.update(clips=clips,summary=result['summary']),history=True)

    def hooks(self,ctx,pid,args):
        p=self.store.load(pid)
        clips=self.selected(p,args)
        for i,clip in enumerate(clips):
            ctx.progress(f"후킹 후보 생성 · {i+1}/{len(clips)} · {clip['title']}",i*100/len(clips))
            result=ai.hooks(ctx,p,clip,self.store.folder(pid)/'ai-cache',args.get('fresh',False))
            def save(project):
                c=next(c for c in project['clips'] if c['id']==clip['id'])
                c.update(hooks=result['hooks'],recommended_index=result['recommended_index'],recommendation_reason=result['reason'])
                if not c['confirmed']:
                    recommended=result['hooks'][result['recommended_index']]
                    c.update(hook=recommended['text'],yellow=recommended['yellow_phrase'])
    ...
    def framing(self,ctx,pid,args):
        ...
        for clip in self.selected(p,args):
            ...
            result=framing.analyze(ctx,p,clip,self.store.folder(pid)/'frames'/clip['id'],self.store.folder(pid)/'ai-cache')
            def save(project):
                c=next(c for c in project['clips'] if c['id']==clip['id'])
    ...
    def edit(self,pid,body):
        if self.jobs.busy(pid):
            raise ValueError('작업 진행 중에는 수정할 수 없습니다. 완료하거나 중단해 주세요.')
        def mutate(p):
            action=body.get('action','update')
            if action=='settings':
                if body['model'] not in ai.models():
                    raise ValueError('사용 가능한 모델을 선택해 주세요.')
                if body['effort'] not in ('low','medium','high','xhigh') or body['stt_model'] not in ('small','medium','large-v3'):
            ...
            if action=='split':
                at=float(body['at'])
                if not c['start']+.1<at<c['end']-.1:
                    raise ValueError('구간 안에서 분할 지점을 선택해 주세요.')
                second=new_clip(at,c['end'],c['title']+' (2)',c['reason'])
                c.update(end=at,hooks=[],hook='',yellow='',confirmed=False,framing=[],render=None)
                p['clips'].insert(p['clips'].index(c)+1,second)
            elif action=='merge':
                i=p['clips'].index(c)
                if i+1>=len(p['clips']): raise ValueError('뒤에 합칠 구간이 없습니다.')
                other=p['clips'][i+1]
                c.update(start=min(c['start'],other['start']),end=max(c['end'],other['end']),hooks=[],hook='',yellow='',confirmed=False,framing=[],render=None)
                p['clips'].pop(i+1)
            elif action=='update':
                allowed=('start','end','title','included','hook','yellow','confirmed','font_size','manual_frame')
                changes={k:v for k,v in body.get('changes',{}).items() if k in allowed}
                bounds=any(k in changes and changes[k]!=c[k] for k in ('start','end'))
                if bounds:
