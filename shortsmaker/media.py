...
FONT_CANDIDATES = [Path.home() / 'Library/Fonts/GmarketSansBold.otf',
                   Path('/System/Library/Fonts/AppleSDGothicNeo.ttc')]


def existing_path(value):
    for spelling in (value, unicodedata.normalize('NFD', value), unicodedata.normalize('NFC', value)):
        p = Path(spelling).expanduser().resolve()
        if p.is_file():
            return p
    raise ValueError('영상 파일을 찾을 수 없습니다. Finder에서 다운로드 상태와 경로를 확인해 주세요.')
...
def probe(path):
    ...
    if r.returncode:
        raise ValueError('영상을 읽을 수 없습니다. 파일 다운로드가 완료되었는지 확인해 주세요.')
    ...
    stat = Path(path).stat()
    return dict(width=w, height=h, duration=duration, fps=v.get('avg_frame_rate', '30/1'),
                has_audio=any(s['codec_type'] == 'audio' for s in data['streams']),
                hdr=v.get('color_transfer') in ('smpte2084', 'arib-std-b67'),
                size=stat.st_size, mtime_ns=stat.st_mtime_ns)


def geometry(metadata, zoom=1.5, center=.5, end_center=None):
    # Retain the entire source height and black title/footer space.
    max_zoom = min(2., 1080 * metadata['width'] / (WIDTH * metadata['height']))
    zoom = max(1., min(float(zoom), max_zoom))
    sw = int(round(WIDTH * zoom / 2)) * 2
    sh = int(round(sw * metadata['height'] / metadata['width'] / 2)) * 2
    if sh > 1080:
        raise ValueError('가로 영상만 지원합니다. 세로 영상은 원본 가로 롱폼을 선택해 주세요.')
    def offset(c):
        return max(0., min(sw - WIDTH, float(c) * sw - WIDTH / 2))
    x = offset(center)
    return dict(zoom=sw / WIDTH, scaled_width=sw, scaled_height=sh,
                x=x, end_x=offset(center if end_center is None else end_center), y=(HEIGHT-sh)//2)
...
def title_image(text, yellow, path, font_size=80):
    ...
    for i, line in enumerate(lines):
        ...
        for j, char in enumerate(line):
            ...
            draw.text((x, top+i*line_height), char, font=font, fill=color, anchor='lt')
...
def render_key(project, clip):
    payload = {'version': 1, 'source': project['metadata'], 'path':project['source'],
               'clip': {k:v for k,v in clip.items() if k in ('start','end','hook','yellow','font_size','framing','manual_frame')}}
...
def scenes_for(clip):
    ...
    scenes = clip.get('framing') or [dict(start=clip['start'], end=clip['end'], zoom=1.5, center=.5)]
    ...
    for s in sorted(scenes, key=lambda s:s['start']):
        ...
        if b <= a:
            continue
        if a > cursor+.001:
            result.append(dict(start=cursor, end=a, zoom=1.5, center=.5))
        a = max(a,cursor)
        if b > a:
            result.append(dict(s, start=a, end=b))
            cursor=b
    if cursor < clip['end']-.001:
        result.append(dict(start=cursor, end=clip['end'],zoom=1.5,center=.5))
    return result


def video_filter(meta, scene, title_input='1:v'):
    g = geometry(meta, scene.get('zoom',1.5),scene.get('center',.5),scene.get('end_center'))
    ...
    hdr = ('zscale=t=linear:npl=100,format=gbrpf32le,zscale=p=bt709,'
           'tonemap=tonemap=hable:desat=0,zscale=t=bt709:m=bt709:r=tv,') if meta.get('hdr') else ''
    return (f"[0:v]setpts=PTS-STARTPTS,{hdr}scale={g['scaled_width']}:{g['scaled_height']},"
...
def export_clip(ctx, project, clip, folder):
    ...
    name = re.sub(r'[^\w가-힣 .-]', '', clip['hook'].replace('\n',' '))[:70].strip() or '쇼츠'
    ...
    for i,scene in enumerate(scenes):
        ...
        args=['ffmpeg','-hide_banner','-loglevel','error','-y','-ss',str(scene['start']),'-i',project['source'],
              '-loop','1','-i',str(title),'-t',str(scene['end']-scene['start']),
              '-filter_complex',video_filter(project['metadata'],scene),'-map','[v]','-map','0:a?',
    ...
    ctx.run(['ffmpeg','-hide_banner','-loglevel','error','-y','-f','concat','-safe','1','-i',str(concat),
             '-c','copy','-movflags','+faststart',str(temp)])
    info=probe(temp)
    if abs(info['duration']-(clip['end']-clip['start']))>.25:
        raise RuntimeError('출력 길이가 구간 길이와 다릅니다. 결과를 확정하지 않았습니다.')
    temp.replace(target)
    for part in parts:
