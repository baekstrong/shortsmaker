...
class FakeConnections:
    def __init__(self):self.posts={};self.created=[];self.deleted=[];self.fail=None
    def channels(self):return [dict(id=x,name=x,displayName=x,service=x,isQueuePaused=False) for x in ('instagram','youtube')]
    def upload(self,path,key,ctx):return 'https://media.example/'+key
    def create(self,channel,text,due,url,privacy):
        self.created.append(channel['id'])
        if channel['id']==self.fail:raise BufferError('network lost')
        p=dict(id=channel['id'],status='scheduled',dueAt=due,sentAt=None)
        self.posts[p['id']]=p
        return p
    def post(self,id):
        if id not in self.posts:raise BufferError('missing','NOT_FOUND')
        return self.posts[id]
    def delete(self,id):self.posts.pop(id)
    def remove_media(self,key):self.deleted.append(key)


def setup(tmp_path):
    store=Store(tmp_path)
    p=store.create('/tmp/video.mp4',dict(duration=300,width=1920,height=1080,size=1,mtime_ns=1))
    c=new_clip(0,100);c.update(hook='매일 무겁게 하면 손해',yellow='손해',confirmed=True)
    output=tmp_path/'render.mp4';output.write_bytes(b'fake')
    ...
    args=dict(clip_ids=[c['id']],channel_ids=['instagram','youtube'],due_at=(datetime.now(timezone.utc)+timedelta(days=7)).isoformat())
...
def test_unknown_outcome_blocks_retry_and_preserves_all_destinations(tmp_path):
    p,publisher,conn,args=setup(tmp_path);conn.fail='youtube'
    with pytest.raises(BufferError):publisher.schedule(Context(),p['id'],args)
    record=publisher.records()[0]
    assert [d['status'] for d in record['deliveries']]==['scheduled','unknown']
    with pytest.raises(ValueError,match='불명확'):publisher.schedule(Context(),p['id'],args)
    assert conn.created==['instagram','youtube']
    assert not conn.deleted


def test_first_channel_failure_keeps_pending_second_channel(tmp_path):
    p,publisher,conn,args=setup(tmp_path);conn.fail='instagram'
    with pytest.raises(BufferError):publisher.schedule(Context(),p['id'],args)
    assert [d['status'] for d in publisher.records()[0]['deliveries']]==['unknown','pending']


def test_delete_verifies_not_found_before_media_deletion(tmp_path):
    p,publisher,conn,args=setup(tmp_path)
    publisher.schedule(Context(),p['id'],args)
    publisher.delete_job(Context(),p['id'],{'record_id':publisher.records()[0]['id']})
